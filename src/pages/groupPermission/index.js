  ...
  <Fragment>
   ...
   <Box sx={{ px: 2, display: "flex", flexDirection: "column", gap: 1, justifyContent: "center", alignItems: "center" }}>
    <DataTable
     ...
     heads={["STT", "Quyền hệ thống", "Mô tả", "Quyền", "Thao tác"].map((head, i) =>
      createCell(head, { sx: { width: i == 0 ? "5%" : i == 1 ? "20%" : i == 4 ? "10%" : "auto", textAlign: "center" } }),
     )}
       ...
       <StyledBox>
        <Box>
         {item.permissions.map((permission, j) => {
          return (
           <Typography variant='h5' sx={{ fontSize: "16px", color: "#143250", fontWeight: 400 }}>
            {j + 1}
            {". "}
            {permission.name}
           </Typography>
