  ...
  const handleUpdate = () => {
   ...
   try {
    updateGroupPermission({
     ...
     name,
     description,
     permissions: ["676675457a0be5f5c4894870", "6766753d7a0be5f5c489486d", "676675587a0be5f5c4894873", "67679524288a1d5c1a8e61e4"],
    }).then((res) => {
     if (_.get(res, "code") === 200) {
     ...
     <Grid container spacing={2}>
      <Grid item xs={6}>
       <Box sx={{border: "1px solid #CCCFD3", borderRadius: "12px"}} display='flex' flexDirection='column' justifyContent='center' alignItems='start' gap='20px' mt={1} p={2}>
        <Typography variant='p' sx={{fontSize: "24px", color: "#2E3236", fontWeight: 700, textAlign: "center", width: "100%"}}>
             ...
             <Box sx={{background: permission.status ? "#E5F1FF" : "#F3F3F3", padding: "12px 16px", borderRadius: "12px", width: "100%"}}>
              <Stack direction='row' spacing={2} sx={{justifyContent: "space-between", alignItems: "center"}}>
               <Stack direction='row' spacing={2} sx={{justifyContent: "flex-start", alignItems: "center"}}>
                <Typography variant='p' sx={{fontSize: "16px", color: "#007CFE", fontWeight: 700}}>
                 {j + 1}
                </Typography>
                <Typography variant='p' sx={{fontSize: "18px", color: "#010810", fontWeight: 400}}>
                 {permission.name}
                </Typography>
               </Stack>
               <Stack direction='row' spacing={2} sx={{justifyContent: "flex-start", alignItems: "center"}}>
