...
import {Avatar, Box, Button, Chip, FormControlLabel, IconButton, Paper, Tooltip, Breadcrumbs, Typography, Stack, Pagination} from "@mui/material"
...
const Manage = () => {
 ...
 return (
  <Fragment>
   ...
   >
    <DataTable
     heads={["STT", "Thông tin thành viên", "Email", "Giới tính", "Đơn vị", "Chức vụ", "Tài khoản"].map((head, i) =>
      createCell(head, {
       sx: {
        width: i == 0 ? "5%" : i == 5 ? "10%" : i == 4 ? "30%" : "auto",
        textAlign: "center",
       },
      }),
     )}
      ...
      return createRows([
       ...
       <StyledBox sx={{justifyContent: "start", flexDirection: "row"}}>
        <Stack direction='row' spacing={2} sx={{justifyContent: "flex-start", alignItems: "center"}}>
         ...
         <Stack direction='column' spacing={1} sx={{justifyContent: "center", alignItems: "flex-start"}}>
          <Link to={`${pathname}/${item._id}`}>
           <Typography sx={{color: "#007CFE", fontSize: "16px"}}>{item.name}</Typography>
          </Link>
          <Typography sx={{color: "#143250", fontSize: "16px"}}>{item.phone}</Typography>
         </Stack>
        </Stack>
       </StyledBox>,
       ...
       <StyledBox>
        {_.get(item, "units", []).map((path) => {
         return _.get(path, "name")
        })}
       </StyledBox>,
       <StyledBox>
        {_.get(item, "positions", []).map((path) => {
         return _.get(path, "name")
        })}
       </StyledBox>,
    ...
    />
    <Pagination color='primary' count={pageCount} page={filter.page + 1} onChange={(e, value) => setFilter({page: value - 1})} shape='rounded' />
   </Box>
  </Fragment>
 )
}
