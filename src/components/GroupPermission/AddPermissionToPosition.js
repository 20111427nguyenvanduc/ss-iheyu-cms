...
const AddPermissionToPosition = ({children, permissionsCurrent, onClose = () => {}, dataPermission = [], textSearch, setTextSearch, onSubmitSearch = () => {}}) => {
   ...
   <Dialog fullWidth={true} maxWidth={"lg"} open={open} onClose={handleClose} scroll={"paper"} PaperProps={{sx: {borderRadius: "16px"}}}>
    ...
    <DialogContent dividers>
     {listCategoryPermission.map((cat, index) => (
      <Box sx={{background: "#F6F5FC", padding: "16px", borderRadius: "16px"}} mt={index ? 2 : 1}>
         ...
         return (
          <Grid item>
           <Box
            ...
            gap={1.5}
            mt={1}
            sx={{background: "#FFF", border: !findIdInArray(permissions, permission._id) ? "1px solid #CCCFD3" : "1px solid #007CFE", borderRadius: "12px"}}
           >
            <Box>
