...
const uploadFile = (postData) => {
 let url = "/fetch/upload-file"
 return promise(url, postData)
}

export function getBase64Image(img, cb) {
 ...
 cb(dataURL.replace(/^data:image\/?[A-z]*;base64,/))
}

export async function getBase64ImageFromURL(url, cb) {
 axios.post("/api/proxy/image", {url}).then((response) => {
  cb(_.get(response, 'data.data'))
 })
}

export {replacePhone, changeAlias, promise, checkLink, msToTime, uploadFile}
