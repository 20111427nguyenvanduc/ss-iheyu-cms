    ...
    <Fragment>
      ...
      {unitsAndPositions.map((item, index) => (
        <Paper key={index} sx={{ p: 2, border: "1px solid #CCCFD3", borderRadius: "12px", mb: 2 }} elevation={0}>
          ...
          <RecursiveSelectDynamic unit={item.unit} position={item.position} setUnit={(value) => handleUpdate(index, "unit", value)} setPosition={(value) => handleUpdate(index, "position", value)} handleUpdatePermissionsAndGroups={handleUpdatePermissionsAndGroups} />
        </Paper>
      ))}
